...
/*bool sendMessage(WiFiClient *client, Sensores *sensor) {
	...
	if (client->connect(_server, 80)) {
		...
		cmd += "&field1=" + String(temp, 2);			// field 1 = Temperature
		cmd += "&field2=" + String(weight, 2);			// field 3 = Weight
		cmd += "&field3=" + String(voltage, 2);			// field 4 = Vcc
		cmd += "\r\n";
...
bool sendMessage(WiFiClient *client, Sensores *sensor, float minDelta) {
...
bool sendMessage(WiFiClient *client, Sensores *sensor, float minDelta, float Delta) {
	...
	if (client->connect(_server, 80)) {
		...
		cmd += "&field2=" + String(weight, 2);			// field 3 = Weight
		cmd += "&field3=" + String(voltage, 2);			// field 4 = Vcc
		cmd += "&field4=" + String(minDelta, 2);		// field 5 = minDelta = weight at 06:00 - readWeight() (every INTERVAL)
		cmd += "&field5=" + String(Delta, 2);			// field 6 = Delta = Weight at 22:00 - weight at 06:00
		cmd += "\r\n";
